public class GaloisField implements Comparable<GaloisField>, Arithmetic{
    private static final int size = 1234567891;
    private int value;
    ...
    // assignment methods
    public void clone(Arithmetic obj) {if(obj instanceof GaloisField) {value = ((GaloisField)obj).value;}}
    public void add(Arithmetic obj) {if(obj instanceof GaloisField) {value = (value + ((GaloisField)obj).value) % size;}}
    public void subtract(Arithmetic obj) {if(obj instanceof GaloisField) {value = (value - ((GaloisField)obj).value + size) % size;}}
    public void multiply(Arithmetic obj) {if(obj instanceof GaloisField) {value = modular_multiplication(value, ((GaloisField)obj).value);}}
    public void divide(Arithmetic obj) {
        if (obj instanceof GaloisField) {
            try {
                int inverse = modular_multiplicative_inverse(((GaloisField)obj).value);
